...
function CameraFeed() {
  ...
  const handleMouseDown = (e) => {
    e.preventDefault(); // Prevents background text selection / scrolling while dragging
    setIsDragging(true);
    offset.current = {
  ...
  return (
    <div
      style={{
        position: 'fixed',
        top: Math.max(0, Math.min(position.y, window.innerHeight - 141)),
        left: Math.max(0, Math.min(position.x, window.innerWidth - 186)),
        width: 180,
        height: 135,
        background: '#000',
        borderRadius: 12,
    ...
    >
      <video
        ...
        playsInline
        muted
        style={{ width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none', transform: 'scaleX(-1)' }}
      />
    </div>
  );
}
